...
그래서 사각지대인 **副詞·接続 / オノマトペ / 複合動詞**를 우선 배치했습니다.
이 선택은 약점이 독해든 청해든 양쪽에 듣습니다 — 副詞·接続은 독해 논리, オノマトペ는 청해에 직결됩니다.

## 기능

- **단어 학습 (일본어 700 + 영어 1500)** — Day당 20개 단위, 간격 반복(SR) 스케줄
  - **類義語 200** (우선순위 1): N1 「言い換え類義」·「用法」 직결. 각 항목이 비슷한 말과의 **차이**와 **함께 쓰이는 말**을 함께 담아요
  - **複合動詞 200** (우선순위 2): 뒤 동사(`〜出す`·`〜込む`·`〜切る`·`〜抜く` 등) 의미 패턴별로 묶어, 처음 보는 복합동사도 뜻을 추측할 수 있게 배치
  - **副詞·接続 200** (우선순위 3): 呼応(호응) 부사가 핵심 — `ろくに`→부정, `まさか`→だろう 처럼 **뒤에 올 문말 형태가 정해진** 부사는 뜻만 외우면 틀립니다. 독해 논리 표지(`とはいえ`·`ひいては`·`もっとも`)도 함께
  - **オノマトペ 100** (우선순위 4): 청해 대비. 비슷하게 들리는 짝(`ぼそぼそ`/`ぶつぶつ`, `くたくた`/`へとへと`)의 차이를 가르는 데 초점
